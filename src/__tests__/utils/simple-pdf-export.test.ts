...
describe('SimplePdfExport', () => {
  describe('generatePrintableHtml', () => {
    ...
    it('should include export date', () => {
      ...
      expect(html).toContain(new Date().toLocaleDateString('en-US', { timeZone: 'UTC' }));
    });
    ...
    it('should remove Confluence-specific classes', () => {
      const contentWithClasses =
        '<div class="confluence-panel"><p class="confluence-text">Test</p></div>';
    ...
    it('should fix relative wiki URLs', () => {
      const contentWithLinks =
        '<img src="/wiki/download/image.png"><a href="/wiki/spaces/DEV">Link</a>';
    ...
    it('should handle special characters in content', () => {
      const contentWithSpecialChars =
        '<p>Test with &amp; special &lt;characters&gt; "quotes" \'apostrophes\'</p>';
      const html = SimplePdfExport.generatePrintableHtml(
    ...
    it('should handle multiple class attributes in content', () => {
      const contentWithMultipleClasses =
        '<div class="class1 class2"><span class="class3">Text</span></div>';
      const html = SimplePdfExport.generatePrintableHtml(
        contentWithMultipleClasses,
        baseTitle,
        baseUrl
      );
    ...
    it('should format dates consistently', () => {
      ...
      expect(html).toContain(currentDate.toLocaleDateString('en-US', { timeZone: 'UTC' }));
      expect(html).toContain(currentDate.toLocaleString('en-US', { timeZone: 'UTC' }));
    });
  });
});
