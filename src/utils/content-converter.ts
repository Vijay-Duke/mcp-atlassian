...
export class ContentConverter {
  ...
  static storageToMarkdown(storage: string): string {
    ...
    markdown = markdown.replace(
      /<ac:structured-macro ac:name="code">.*?<ac:parameter ac:name="language">(.*?)<\/ac:parameter>.*?<ac:plain-text-body><!\[CDATA\[([\s\S]*?)\]\]><\/ac:plain-text-body>.*?<\/ac:structured-macro>/gi,
      (match, lang, code) => {
        return `\`\`\`${lang}\n${code}\n\`\`\``;
      }
    );
    ...
    markdown = markdown.replace(/<script\b[^>]*>.*?<\/script>/gis, '');
    // Remove any remaining opening script tags
    markdown = markdown.replace(/<script\b[^>]*>/gi, '');
